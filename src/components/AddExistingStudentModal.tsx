...
export const AddExistingStudentModal: React.FC<AddExistingStudentModalProps> = ({
  ...
  if (lessonOption === 'none') {
    ...
    numLessonFee = activeSchool?.feeSchedule?.lessonFeeMonthly ?? defaultLessonMonthly;
  } else if (lessonOption === '2_months') {
    numLessonFee = (activeSchool?.feeSchedule?.lessonFeeMonthly ?? defaultLessonMonthly) * 2;
  } else if (lessonOption === 'termly') {
    numLessonFee = activeSchool?.feeSchedule?.lessonFeeTermly ?? defaultLessonTermly;
  ...
  const handleLessonOptionChange = (option: 'none' | '1_month' | '2_months' | 'termly' | 'custom') => {
    ...
    const mRate = activeSchool?.feeSchedule?.lessonFeeMonthly ?? defaultLessonMonthly;
    const tRate = activeSchool?.feeSchedule?.lessonFeeTermly ?? defaultLessonTermly;
    if (option === 'none') {
      setLessonFeeCustom('0');
      setLessonPaid('0');
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl w-full max-w-md max-h-[90vh] flex flex-col shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        {/* Header */}
