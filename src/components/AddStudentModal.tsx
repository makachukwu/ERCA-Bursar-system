...
export const AddStudentModal: React.FC<AddStudentModalProps> = ({
  ...
  // Optional Lesson Fee
  const [lessonOption, setLessonOption] = useState<'none' | '1_month' | '2_months' | 'termly'>('none');
  ...
  const handlePayAllInFull = () => {
    setAdmissionPaid(String(parsedAdmissionFee || 4000));
    setExamPaid(String(parsedExamFee || 1000));
    if (!isExemptFromSchoolFee) {
      setSchoolPaid(String(rawSchoolFee || 5000));
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (lessonOption !== 'none' && parsedLessonPaid < parsedLessonFee) {
      setErrorMessage(`Lesson fee must be paid before admitting when lesson is included.`);
      return;
    }

    if (!hasAgreedPayment) {
      setErrorMessage('You must confirm and agree that the student has paid the admission and prerequisite fees before enrolling.');
      return;
    }
    ...
    try {
      ...
      const newRecord: StudentPaymentRecord = {
        ...
        // Optional Lesson Fee Column
        lesson_fee: parsedLessonFee,
        lesson_paid: parsedLessonPaid,
        lesson_status: parsedLessonFee > 0 ? (parsedLessonPaid >= parsedLessonFee ? 'fully_paid' : 'part_payment') : 'unpaid',
        lesson_months: parsedLessonFee > 0 
          ? (lessonOption === 'termly' ? 'Full Term (Paid)' : lessonOption === '2_months' ? '2 Months (Paid)' : '1 Month (Paid)')
          : 'None',
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/40 backdrop-blur-xs">
      <div className="w-full max-w-lg bg-white rounded-t-[32px] sm:rounded-3xl max-h-[94vh] flex flex-col shadow-2xl border border-[#f0f0f0] overflow-hidden animate-in slide-in-from-bottom duration-200">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-4 flex-1">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div>
              ...
              <input
                ...
                placeholder="Enter ID manually (e.g. STD-001)"
          ...
          <div className="bg-white rounded-2xl p-4 border border-[#e8e8ed] space-y-2.5 shadow-xs">
            <div className="flex items-center justify-between pb-1 border-b border-[#f0f0f0]">
              ...
              <span className="text-[10px] font-bold text-[#777]">Paid before admitting</span>
            </div>
