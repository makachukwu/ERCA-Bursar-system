...
export const StudentDetailsModal: React.FC<StudentDetailsModalProps> = ({
  ...
  const [examFee, setExamFee] = useState<string>('1000');
  const [examPaid, setExamPaid] = useState<string>('0');
  const [lessonFee, setLessonFee] = useState<string>('6000');
  ...
  useEffect(() => {
    if (student && isOpen) {
      ...
      if (prevInitKeyRef.current !== currentKey) {
        ...
        setExamFee(String(student.exam_fee !== undefined && student.exam_fee !== null ? student.exam_fee : (bd.examFee || 0)));
        setExamPaid(String(student.exam_paid !== undefined ? student.exam_paid : 0));
        setLessonFee(String(student.lesson_fee !== undefined && student.lesson_fee !== null ? student.lesson_fee : (bd.lessonFee || 0)));
        ...
        setTuitionFee(String(student.tuition_fee !== undefined && student.tuition_fee !== null ? student.tuition_fee : bd.tuitionFee));
        setTuitionPaid(String(student.tuition_paid !== undefined && student.tuition_paid !== null ? student.tuition_paid : bd.tuitionPaid));
        setAdmissionFee(String(student.is_new_admission === false ? 0 : (student.admission_fee !== undefined ? student.admission_fee : (student.is_new_admission ? bd.admissionFee : 0))));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/50">
      <div className="w-full max-w-md sm:max-w-xl bg-white rounded-t-[28px] sm:rounded-3xl max-h-[88vh] sm:max-h-[92vh] flex flex-col shadow-2xl border border-[#f0f0f0] overflow-hidden animate-in slide-in-from-bottom duration-200">
        {/* Header */}
            ...
            <div className="space-y-4 bg-[#f4f4f7] p-5 rounded-3xl border border-[#eee]">
              ...
              <div className="space-y-2 text-xs bg-white p-4 rounded-2xl border border-[#eee] font-mono">
                ...
                <div className="flex justify-between py-1 border-b border-[#f0f0f0]">
                  ...
                  <span className="font-bold text-[#1a1a1a]">
                    {formatCurrency(student.lesson_fee ?? 6000, currencySymbol)} / {formatCurrency(student.lesson_paid ?? 0, currencySymbol)} →{' '}
            ...
            <div className="space-y-4">
                        ...
                        <p className="text-sm font-black text-[#1a1a1a]">
                          {formatCurrency(
                            breakdown.admissionFee > 0 
                              ...
                              : (student.admission_fee ?? (student.is_new_admission ? (activeSchool?.feeSchedule?.admissionFee ?? 5000) : 0)), 
                            currencySymbol
                          )}
                        </p>
              ...
              <div className="bg-[#f4f4f7] rounded-3xl p-5 border border-[#eee] space-y-2 text-xs">
                ...
                <div className="flex justify-between py-1 border-b border-[#eee]">
                  ...
                  <span className="font-bold text-[#1a1a1a]">
                    {formatCurrency(student.admission_fee ?? (student.is_new_admission ? 4000 : 0), currencySymbol)} / {formatCurrency(student.admission_paid ?? 0, currencySymbol)}
                  </span>
                </div>
                ...
                <div className="flex justify-between py-1 border-b border-[#eee]">
                  ...
                  <span className="font-bold text-[#1a1a1a]">
                    {formatCurrency(student.lesson_fee ?? 6000, currencySymbol)} / {formatCurrency(student.lesson_paid ?? 0, currencySymbol)}
                    {student.lesson_months ? ` (${student.lesson_months})` : ''}
                  </span>
                </div>
