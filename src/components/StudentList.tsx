...
import { formatCurrency, sortClassesInAcademicOrder, deriveFeeBreakdown } from '../services/calculations';
  ...
  const metrics = useMemo(() => {
    ...
    if (Array.isArray(filteredStudents)) {
      filteredStudents.forEach((s) => {
        if (!s) return;
        const bd = deriveFeeBreakdown(s, effectiveSchedule);
    ...
    const overallExpected = schoolFeeExpected + admissionFeeExpected + lessonFeeExpected + examFeeExpected;
    const overallBalance = Math.max(0, overallExpected - overallCollected);

    const schoolFeeBalance = Math.max(0, schoolFeeExpected - schoolFeeCollected);
    const admissionFeeBalance = Math.max(0, admissionFeeExpected - admissionFeeCollected);
    const lessonFeeBalance = Math.max(0, lessonFeeExpected - lessonFeeCollected);
    const examFeeBalance = Math.max(0, examFeeExpected - examFeeCollected);

    const overallRate = overallExpected > 0 ? Math.min(100, (overallCollected / overallExpected) * 100) : 0;
    const schoolFeeRate = schoolFeeExpected > 0 ? Math.min(100, (schoolFeeCollected / schoolFeeExpected) * 100) : 0;
    const admissionFeeRate = admissionFeeExpected > 0 ? Math.min(100, (admissionFeeCollected / admissionFeeExpected) * 100) : 0;
    const lessonFeeRate = lessonFeeExpected > 0 ? Math.min(100, (lessonFeeCollected / lessonFeeExpected) * 100) : 0;
    const examFeeRate = examFeeExpected > 0 ? Math.min(100, (examFeeCollected / examFeeExpected) * 100) : 0;
    ...
    <div className="flex-1 pb-28 px-5 pt-4 space-y-4">
      ...
      {students.length > 0 && (
        ...
        >
          ...
          {/* Bottom Grid: 4 Dedicated Fee Collection Cards (School Fee, Exam Fee, Lesson Fee, Admission Fee) */}
            ...
            <div className="bg-white/10 hover:bg-white/15 transition-colors rounded-2xl p-3 flex flex-col justify-between border border-white/15">
              ...
              <div className="space-y-0.5">
                ...
                <div className="flex items-center justify-between text-[9px] text-blue-200/90 font-medium">
                  <span>Target: <span className="font-mono">{formatCurrency(metrics.schoolFeeExpected, currencySymbol)}</span></span>
                </div>
                <div className="text-[9px] text-amber-300/90 font-semibold truncate font-mono">
                  Due: {formatCurrency(metrics.schoolFeeBalance, currencySymbol)}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/10 hover:bg-white/15 transition-colors rounded-2xl p-3 flex flex-col justify-between border border-white/15">
              ...
              <div className="space-y-0.5">
                ...
                <div className="flex items-center justify-between text-[9px] text-amber-200/90 font-medium">
                  <span>Target: <span className="font-mono">{formatCurrency(metrics.examFeeExpected, currencySymbol)}</span></span>
                </div>
                <div className="text-[9px] text-amber-300/90 font-semibold truncate font-mono">
                  Due: {formatCurrency(metrics.examFeeBalance, currencySymbol)}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/10 hover:bg-white/15 transition-colors rounded-2xl p-3 flex flex-col justify-between border border-white/15">
              ...
              <div className="space-y-0.5">
                ...
                <div className="flex items-center justify-between text-[9px] text-emerald-200/90 font-medium">
                  <span>Target: <span className="font-mono">{formatCurrency(metrics.lessonFeeExpected, currencySymbol)}</span></span>
                </div>
                <div className="text-[9px] text-amber-300/90 font-semibold truncate font-mono">
                  Due: {formatCurrency(metrics.lessonFeeBalance, currencySymbol)}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/10 hover:bg-white/15 transition-colors rounded-2xl p-3 flex flex-col justify-between border border-cyan-300/30">
              ...
              <div className="space-y-0.5">
                ...
                <div className="flex items-center justify-between text-[9px] text-cyan-100/90 font-medium">
                  <span>Target: <span className="font-mono">{formatCurrency(metrics.admissionFeeExpected, currencySymbol)}</span></span>
                </div>
                <div className="text-[9px] text-cyan-200/90 font-semibold truncate font-mono">
                  Due: {formatCurrency(metrics.admissionFeeBalance, currencySymbol)}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-3xl p-8 border border-[#f0f0f0] text-center space-y-4 shadow-sm my-4">
          ...
          <div>
            ...
            <p className="text-xs text-[#666] mt-1 max-w-sm mx-auto leading-relaxed">
              Transfer your entire student roster directly from Google Sheets or Excel to Firebase Cloud in seconds, or add individual students.
            </p>
          </div>
          <div className="flex items-center justify-center gap-3 flex-wrap pt-1">
            {onOpenMigrator && (
              <button
                ...
                id="empty-state-migrate-btn"
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-blue-600 text-white text-xs font-bold hover:bg-blue-700 active:scale-95 transition-all shadow-sm cursor-pointer"
              >
                <Sparkles className="w-4 h-4 text-amber-300" />
                <span>Migrate from Google Sheets / Excel</span>
              </button>
            )}
            <button
              onClick={onOpenAddStudent}
