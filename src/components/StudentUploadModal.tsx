...
interface StudentUploadModalProps {
  ...
  onUploadSuccess: (newStudents: StudentPaymentRecord[], message: string) => void;
}
...
export const StudentUploadModal: React.FC<StudentUploadModalProps> = ({
  ...
  const normalizeRow = (rawRow: Record<string, any>, index: number): StudentPaymentRecord | null => {
    ...
    if (!studentId || studentId === '0' || studentId === '-') {
      const count = existingStudents.length + index + 1;
    ...
    const totalFeeRaw = findRowValue(rawRow, ['total_fee', 'fee_amount', 'expected_fee', 'fees', 'amount_due', 'bill', 'total_amount', 'total_fees', 'school_fees', 'fee', 'total']);
  ...
  const handleCommitUploadToFirebase = async () => {
    ...
    try {
      ...
      // 1. Immediately save to local storage cache (guarantees zero data loss even if offline)
      ...
      // Even if cloud write encounters a transient issue, local storage has been updated
      onUploadSuccess(
        parsedStudents,
        `Saved ${parsedStudents.length} students to local database and queued for cloud sync.`
      );
      onClose();
    } finally {
