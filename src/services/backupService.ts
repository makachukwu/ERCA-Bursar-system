...
export function downloadComprehensiveExcelBackup(
  ...
  const schoolSlug = (session.schoolName || 'eminent_academy').toLowerCase().replace(/[^a-z0-9]+/g, '_');
...
export function saveLocalSnapshot(
  ...
  const snapshot: TermSnapshot = {
    ...
    schoolName: session.schoolName || 'Eminent Royal Crown Academy',
  ...
  const targetSchoolId = schoolId || session.activeSchoolId || session.schoolId || 'eminent-academy';
...
export function getSavedSnapshots(schoolId?: string): TermSnapshot[] {
  const targetSchoolId = schoolId || 'eminent-academy';
  const targetKey = getSchoolSnapshotsStorageKey(targetSchoolId);
