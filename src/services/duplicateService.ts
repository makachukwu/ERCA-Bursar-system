 ...
 */
export function getStoredIgnoredDuplicateIds(schoolId: string = 'eminent-academy'): string[] {
  try {
 ...
 */
export function saveStoredIgnoredDuplicateIds(ignoredIds: string[], schoolId: string = 'eminent-academy'): void {
  try {
...
export function detectDuplicateGroups(
  ...
  groupsMap.forEach((val, groupId) => {
    ...
    val.records.forEach((rec) => {
      ...
      const totalPaid = (Number(rec.amount_paid) || 0) + 
...
export function mergeStudentRecords(
  ...
  records.forEach((r) => {
    ...
    sumTuitionPaid += (Number(r.amount_paid) || Number(r.tuition_paid) || 0);
...
export async function resolveDuplicateGroup(
  ...
  // 1. Remove deleted records from student array and mark them as deleted in tombstone registry
  ...
  // 2. Update or ensure the primary record is present in remaining
  const finalStudents = remaining.map((s) => (s.id === primaryId ? finalRecord : s));

  // 3. Save to local storage
  saveStoredStudents(finalStudents, schoolId);

  // 4. Perform Firestore cloud operations
  saveStudentToFirestore(finalRecord, schoolId).catch(() => {});
  for (const rec of recordsToDelete) {
    deleteStudentFromFirestore(rec.id, schoolId).catch(() => {});
  }
