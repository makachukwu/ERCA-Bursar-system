 ...
 */
export function getStoredExpenses(schoolId: string = 'eminent-academy'): ExpenseItem[] {
  try {
 ...
 */
export function saveStoredExpenses(expenses: ExpenseItem[], schoolId: string = 'eminent-academy'): void {
  try {
    const key = `${STORAGE_EXPENSES_PREFIX}_${schoolId}`;
    safeStorage.setItem(key, JSON.stringify(expenses));
    batchSaveExpensesToFirestore(expenses, schoolId).catch(() => {});
  } catch (e) {
 ...
 */
export function addExpenseItem(expense: Omit<ExpenseItem, 'id' | 'createdAt'>, schoolId: string = 'eminent-academy'): ExpenseItem {
  ...
  saveExpenseToFirestore(newExpense, schoolId).catch(() => {});
  return newExpense;
}
 ...
 */
export function deleteExpenseItem(expenseId: string, schoolId: string = 'eminent-academy'): ExpenseItem[] {
  const current = getStoredExpenses(schoolId);
  const updated = current.filter((e) => e.id !== expenseId);
  saveStoredExpenses(updated, schoolId);
  deleteExpenseFromFirestore(expenseId, schoolId).catch(() => {});
  return updated;
}
