...
/**
 * Initializes Firebase App and Firestore with persistent multi-tab local caching and auto long polling
 */
...
export async function saveStudentToFirestore(
  ...
  const firestore = getFirebaseDb();
  if (!firestore || !student.id) return false;
...
export async function deleteStudentFromFirestore(
  ...
  const firestore = getFirebaseDb();
  if (!firestore || !studentId) return false;
...
export async function saveScholarshipToFirestore(
  ...
  const firestore = getFirebaseDb();
  if (!firestore || !scholarship.id) return false;
...
export async function deleteScholarshipFromFirestore(
  ...
  const firestore = getFirebaseDb();
  if (!firestore || !scholarshipId) return false;
...
export async function saveExpenseToFirestore(
  ...
  const firestore = getFirebaseDb();
  if (!firestore || !expense.id) return false;
...
export async function deleteExpenseFromFirestore(
  ...
  const firestore = getFirebaseDb();
  if (!firestore || !expenseId) return false;
...
export async function saveStaffToFirestore(
  ...
  const firestore = getFirebaseDb();
  if (!firestore || !staff.id) return false;
...
export async function deleteStaffFromFirestore(
  ...
  const firestore = getFirebaseDb();
  if (!firestore || !staffId) return false;
...
export async function savePayrollToFirestore(
  ...
  const firestore = getFirebaseDb();
  if (!firestore || !payroll.id) return false;
...
export async function deletePayrollFromFirestore(
  ...
  const firestore = getFirebaseDb();
  if (!firestore || !payrollId) return false;
...
export async function saveRemittanceToFirestore(
  ...
  const firestore = getFirebaseDb();
  if (!firestore || !remittance.id) return false;
 ...
 */
export async function getFullSchoolDataFromFirestore(schoolId: string = 'eminent-academy') {
  const targetSchool = schoolId.toLowerCase();
  const [
    students,
    scholarships,
