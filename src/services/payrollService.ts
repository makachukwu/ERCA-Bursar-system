 ...
 */
export function getStoredStaff(schoolId: string = 'eminent-academy'): StaffMember[] {
  try {
 ...
 */
export function clearStoredStaffAndPayroll(schoolId: string = 'eminent-academy'): void {
  try {
...
/**
 * Save staff roster for a specific school to storage
 */
export function saveStoredStaff(staff: StaffMember[], schoolId: string = 'eminent-academy'): void {
  try {
    const key = `${STORAGE_STAFF_PREFIX}_${schoolId}`;
    safeStorage.setItem(key, JSON.stringify(staff));
  } catch (e) {
    console.warn(`[Payroll Storage Note] Failed saving staff for '${schoolId}':`, e);
  }
}
 ...
 */
export function getStoredPayrollRecords(schoolId: string = 'eminent-academy'): PayrollRecord[] {
  try {
    const key = `${STORAGE_PAYROLL_PREFIX}_${schoolId}`;
    const stored = safeStorage.getItem(key);
    if (stored) {
      ...
      if (Array.isArray(parsed)) {
        ...
        if (clean.length !== parsed.length) {
          saveStoredPayrollRecords(clean, schoolId);
        }
        return clean;
      }
    }
  } catch (e) {
    console.warn(`[Payroll Storage Note] Failed loading payroll records for '${schoolId}':`, e);
  }
  return [];
}

/**
 * Save payroll payment records for a specific school to storage
 */
export function saveStoredPayrollRecords(records: PayrollRecord[], schoolId: string = 'eminent-academy'): void {
  try {
    const key = `${STORAGE_PAYROLL_PREFIX}_${schoolId}`;
    safeStorage.setItem(key, JSON.stringify(records));
  } catch (e) {
    console.warn(`[Payroll Storage Note] Failed saving payroll records for '${schoolId}':`, e);
  }
}
...
export function calculateMonthlyPayrollSummary(
  ...
  const monthRecords = payrollRecords.filter((r) => r.month === targetMonth);
